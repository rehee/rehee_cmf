...
namespace ReheeCmf.Helpers
{
	public static class ContentResponseHelper
	{
		...
		public static T SetError<T>(this T that, Error? error) where T : IContentResponse
		{
			that.Status = error?.Status ?? HttpStatusCode.BadRequest;
			that.ErrorMessage = error?.Message;
			that.ErrorCode = error?.ErrorCode;
