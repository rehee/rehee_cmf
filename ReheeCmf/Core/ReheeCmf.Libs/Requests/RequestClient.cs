namespace ReheeCmf.Requests
{
	public class RequestClient : RequestBase, IRequestClient
	{
		...
		public async Task<ContentResponse<T>> Request<T>(HttpMethod method, string url, string? json = null, Stream? content = null, Dictionary<string, string>? headValue = null, CancellationToken ct = default, bool ignoreToken = false)
		{
			var tokens = ignoreToken ? (null, null) : await clientTokenService.GetRequestTokenAsync(ct);
			return await Request<T>(method, url, json, content, headValue, tokens.name, tokens.token, ct, ignoreToken);
		}
	}
