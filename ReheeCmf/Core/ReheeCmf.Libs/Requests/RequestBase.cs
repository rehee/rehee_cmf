...
namespace ReheeCmf.Requests
{
	public abstract class RequestBase : IRequestBase
	{
		...
		public async Task<ContentResponse<T>> Request<T>
			...
			string? name = null, string? token = null, CancellationToken ct = default, bool ignoreToken = false)
		{
			var result = new ContentResponse<T>();
			using var client = GetHttpClient(name);

			var response = await this.RequestString(client, method, url, json, content, headValue, token, ct, ignoreToken);
			if (response.Success)
			{
				try
				{
			...
			else
			{
				try
				{
					var error = JsonSerializer.Deserialize<Error>(response.Content ?? "", JsonOption.DefaultOption);
					result.SetError(error);
				}
				catch
				{
					result.ValidationResult = JsonSerializer.Deserialize<ValidationResultDTO[]>(response.Content ?? "", JsonOption.DefaultOption);
				}
			}
		...
		private async Task<ContentResponse<string>> RequestString(
			...
			string? token = null, CancellationToken ct = default, bool ignoreToken = false)
		{
