...
namespace ReheeCmf.Tenants
{
	...
	public class TenantEntityHandler : EntityChangeHandler<TenantEntity>
	{
		...
		{
			var baseValidation = base.Validation().Concat(getBasicValidation(entity!, context!));
			var tenantValidation = sp?.GetService<ITenantEntityValidation>();

			if (tenantValidation != null && entity != null && context != null)
			{
				var validation = tenantValidation.Validate(entity, context);
