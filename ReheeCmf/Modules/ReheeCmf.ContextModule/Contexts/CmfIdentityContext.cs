...
{
	public interface IWithTrackerCallback
	{
		Dictionary<Guid, ICrudTracker> CrudTrackers { get; set; }
	}
	...
	public class CmfIdentityContext<TUser> : IdentityDbContext<
		...
		TenantIdentityUserToken>, IIdentityContext, IWithContext, ITenantContext, ITokenDTOContext, IWithTrackerCallback
		where TUser : IdentityUser, ICmfUser, new()
	{
		...
		public Dictionary<Guid, ICrudTracker> CrudTrackers { get; set; } = new Dictionary<Guid, ICrudTracker>();
		public override int SaveChanges()
		{
			var result = base.SaveChanges();
			Task.Run(async () =>
			{
				foreach (var tracker in CrudTrackers ?? [])
				{
					if (tracker.Value != null)
					{
						await tracker.Value.AfterSaveChangesAsync();
					}
				}
				CrudTrackers?.Clear();
			});
